...
for L in range(0, max_pruning_level):
    ...
    if L > 0:
        print("Did model stay the same? Expected: No", compare_models(prev_model, best_model))
    early_stopper.reset()
    ...
    torch.save(best_model, "tmp.pth")
    save_model = torch.load("tmp.pth")
    models.append(save_model)

    prev_model = torch.load("tmp.pth")
    print("Expected: Yes", compare_models(best_model, prev_model))
